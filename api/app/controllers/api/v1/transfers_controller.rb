module Api
  module V1
    class TransfersController < ApplicationController
      ...
      def index
        transfers = financial_household.transfers.includes(:financial_transactions).where(source_account_id: visible_accounts).or(financial_household.transfers.includes(:financial_transactions).where(destination_account_id: visible_accounts)).order(created_at: :desc)
      ...
      def visible_transfer
        financial_household.transfers.includes(:financial_transactions).where(source_account_id: visible_accounts).or(financial_household.transfers.includes(:financial_transactions).where(destination_account_id: visible_accounts)).find(params[:id])
      end
