...
MVP account and transfer currencies SHALL equal the household base currency. Monetary storage SHALL use `numeric(19,4)`, exchange rates `numeric(20,10)` for future use, Rails calculations `BigDecimal`, `BigDecimal::ROUND_HALF_EVEN` when rounding is later needed, and JSON decimal strings. Client idempotency keys SHALL be unique per household and operation scope and repeat requests SHALL return the original result. No Float or live exchange-rate provider is allowed.
